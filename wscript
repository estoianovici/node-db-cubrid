...
def configure(conf):
  ...
  # CUBRID flags and libraries
  conf.env.append_unique('CXXFLAGS', '-I/home/acasa/CUBRID/include/')
  conf.env.append_unique('LINKFLAGS', '-L/home/acasa/CUBRID/lib/libcascci.so')
  conf.check_cxx(lib="libcascci.so", errmsg="Missing libcascci")

def build(bld):
  obj = bld.new_task_gen("cxx", "shlib", "node_addon")
  obj.target = "cubrid_bindings"
  obj.source = "lib/node-db/binding.cc lib/node-db/connection.cc lib/node-db/events.cc lib/node-db/exception.cc lib/node-db/query.cc lib/node-db/result.cc src/connection.cc src/cubrid.cc src/query.cc src/result.cc src/cubrid_bindings.cc"
  obj.includes = "lib/"
  obj.uselib = "libcascci"
...
def lint(lnt):
  ...
  Utils.exec_command('nodelint ./package.json ./db-cubrid.js')
